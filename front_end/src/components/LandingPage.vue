<template>
  <div class="mt-6 hero is-fullheight">
    <!--Squares-->
    <div class="square is-light square-1"></div>
    <div class="square is-light square-2"></div>
    <div class="square is-primary square-3"></div>
    <div class="square is-primary square-4"></div>
    <div class="square is-primary square-5"></div>
    <div class="square is-primary square-6"></div>
    <div class="square is-light square-7"></div>
    <div class="square is-light square-8"></div>

    <div class="hero-body">
      <div class="container">
        <div class="search-hero has-text-centered">
          <h1 class="title is-1 is-bold">Search for Resources</h1>
          <div class="field is-grouped">
     
            <p class="control is-expanded">
              <input
                @click.prevent='query()'
                class="input"
                type="text"
                placeholder="Find a repository"
              />
            </p>
        

            <p class="mt-2 control"></p>
          </div>
          <h3 class="has-text-centered">Or <b>Browse</b> categories</h3>
        </div>

        <div class="container category">
          <div class="columns">
            <div class="column is-4">
              <section class="hero">
                <div class="hero-body">
                  <div class="container">
                    <div class="box">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="ionicon"
                        viewBox="0 0 512 512"
                      >
                        <title>Bar Chart</title>
                        <path
                          d="M32 32v432a16 16 0 0016 16h432"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                        <rect
                          x="96"
                          y="224"
                          width="80"
                          height="192"
                          rx="20"
                          ry="20"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                        <rect
                          x="240"
                          y="176"
                          width="80"
                          height="240"
                          rx="20"
                          ry="20"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                        <rect
                          x="383.64"
                          y="112"
                          width="80"
                          height="304"
                          rx="20"
                          ry="20"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                      </svg>
                      <h1 class="title">
                        Geek for Geeks
                      </h1>

                      <p class="is-family-monospace">
                        'GeeksforGeeks' is a computer science portal.it was
                        created
                      </p>
                    </div>
                  </div>
                </div>
              </section>
            </div>
            <div class="column is-4">
              <section class="hero">
                <div class="hero-body">
                  <div class="container">
                    <div class="box">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="ionicon"
                        viewBox="0 0 512 512"
                      >
                        <title>Settings</title>
                        <path
                          d="M262.29 192.31a64 64 0 1057.4 57.4 64.13 64.13 0 00-57.4-57.4zM416.39 256a154.34 154.34 0 01-1.53 20.79l45.21 35.46a10.81 10.81 0 012.45 13.75l-42.77 74a10.81 10.81 0 01-13.14 4.59l-44.9-18.08a16.11 16.11 0 00-15.17 1.75A164.48 164.48 0 01325 400.8a15.94 15.94 0 00-8.82 12.14l-6.73 47.89a11.08 11.08 0 01-10.68 9.17h-85.54a11.11 11.11 0 01-10.69-8.87l-6.72-47.82a16.07 16.07 0 00-9-12.22 155.3 155.3 0 01-21.46-12.57 16 16 0 00-15.11-1.71l-44.89 18.07a10.81 10.81 0 01-13.14-4.58l-42.77-74a10.8 10.8 0 012.45-13.75l38.21-30a16.05 16.05 0 006-14.08c-.36-4.17-.58-8.33-.58-12.5s.21-8.27.58-12.35a16 16 0 00-6.07-13.94l-38.19-30A10.81 10.81 0 0149.48 186l42.77-74a10.81 10.81 0 0113.14-4.59l44.9 18.08a16.11 16.11 0 0015.17-1.75A164.48 164.48 0 01187 111.2a15.94 15.94 0 008.82-12.14l6.73-47.89A11.08 11.08 0 01213.23 42h85.54a11.11 11.11 0 0110.69 8.87l6.72 47.82a16.07 16.07 0 009 12.22 155.3 155.3 0 0121.46 12.57 16 16 0 0015.11 1.71l44.89-18.07a10.81 10.81 0 0113.14 4.58l42.77 74a10.8 10.8 0 01-2.45 13.75l-38.21 30a16.05 16.05 0 00-6.05 14.08c.33 4.14.55 8.3.55 12.47z"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                      </svg>
                      <h1 class="title">
                        Geek for Geeks
                      </h1>

                      <p class="is-family-monospace">
                        'GeeksforGeeks' is a computer science portal.it was
                        created
                      </p>
                    </div>
                  </div>
                </div>
              </section>
            </div>
            <div class="column is-4">
              <section class="hero">
                <div class="hero-body">
                  <div class="container">
                    <div class="box">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="ionicon"
                        viewBox="0 0 512 512"
                      >
                        <title>Layers</title>
                        <path
                          d="M434.8 137.65l-149.36-68.1c-16.19-7.4-42.69-7.4-58.88 0L77.3 137.65c-17.6 8-17.6 21.09 0 29.09l148 67.5c16.89 7.7 44.69 7.7 61.58 0l148-67.5c17.52-8 17.52-21.1-.08-29.09zM160 308.52l-82.7 37.11c-17.6 8-17.6 21.1 0 29.1l148 67.5c16.89 7.69 44.69 7.69 61.58 0l148-67.5c17.6-8 17.6-21.1 0-29.1l-79.94-38.47"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                        <path
                          d="M160 204.48l-82.8 37.16c-17.6 8-17.6 21.1 0 29.1l148 67.49c16.89 7.7 44.69 7.7 61.58 0l148-67.49c17.7-8 17.7-21.1.1-29.1L352 204.48"
                          fill="none"
                          stroke="currentColor"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                          stroke-width="32"
                        />
                      </svg>
                      <h1 class="title">
                        Geek for Geeks
                      </h1>

                      <p class="is-family-monospace">
                        'GeeksforGeeks' is a computer science portal.it was
                        created
                      </p>
                    </div>
                  </div>
                </div>
              </section>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="hero-foot">
      <h3 class="has-text-centered"><b>Quick</b> Answers</h3>
    </div>
  </div>
</template>

<script lang="ts">
import { Options, Vue } from "vue-class-component";
import axios from 'axios';
import {ref} from 'vue';
import router from "@/router/index.ts";


export default {
    setup() {
      const q = 'alo'
        const query = () => {
        router.push({ path: `/q/${q}` });
        }
        return {
            query
        }
    }
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>
.search-hero h3 {
  font-family: "Montserrat", sans-serif;
  padding: 24px 0;
}

.category .box {
  background: #fff;
  border: 1px solid #e3e3e3;
  border-radius: 10px;
  -webkit-box-shadow: -1px 3px 15px 0 rgba(0, 0, 0, 0.06);
  box-shadow: -1px 3px 15px 0 rgba(0, 0, 0, 0.06);
  text-align: center;
}
.category .box svg {
  display: block;
  max-width: 60px;
  margin: 0 auto 20px auto;
  color: #318c63;
}

.square {
  position: absolute;
}

.square.is-light {
  background: #f5faff;
}

.square.is-primary {
  background: #e4fbf3;
}

.square.square-1 {
  top: 13%;
  left: 19%;
  height: 60px;
  width: 60px;
  -webkit-transform: rotate(32deg);
  transform: rotate(32deg);
  border-radius: 50%;
}

.square.square-2 {
  top: 24%;
  left: -100px;
  height: 200px;
  width: 200px;
  -webkit-transform: rotate(45deg);
  transform: rotate(45deg);
  border-radius: 50%;
}

.square.square-3 {
  top: 36%;
  right: -70px;
  height: 160px;
  width: 160px;
  -webkit-transform: rotate(38deg);
  transform: rotate(38deg);
  border-radius: 50%;
}

.square.square-4 {
  top: 22%;
  right: 14%;
  height: 60px;
  width: 60px;
  -webkit-transform: rotate(76deg);
  transform: rotate(76deg);
  border-radius: 50%;
}

.square.square-5 {
  top: 35%;
  right: 17%;
  height: 32px;
  width: 32px;
  -webkit-transform: rotate(68deg);
  transform: rotate(59deg);
  border-radius: 50%;
}

.square.square-6 {
  bottom: 60px;
  left: 24%;
  height: 32px;
  width: 32px;
  -webkit-transform: rotate(68deg);
  transform: rotate(59deg);
  border-radius: 50%;
}

.square.square-7 {
  bottom: 48px;
  left: 30%;
  height: 26px;
  width: 26px;
  -webkit-transform: rotate(21deg);
  transform: rotate(21deg);
  border-radius: 50%;
}

.square.square-8 {
  bottom: 150px;
  left: 70%;
  height: 26px;
  width: 26px;
  -webkit-transform: rotate(21deg);
  transform: rotate(21deg);
  border-radius: 50%;
}
</style>
